    ...
    setCampaign((prev: any) => {
      ...
      if (newConfig.config.roulette.segments) {
        newConfig.config.roulette.segments = newConfig.config.roulette.segments.map((segment: any, index: number) => ({
          ...segment,
          color: index % 2 === 0 ? 
            (colorIndex === 1 ? color : newConfig.config.roulette.segmentColor1 || segmentColor1) : 
            (colorIndex === 2 ? color : newConfig.config.roulette.segmentColor2 || segmentColor2)
        }));
      }
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-2">
            <label className="text-sm font-medium text-gray-700">Couleur segments 1</label>
            <div className="flex items-center space-x-2">
              <input 
                type="color" 
                value={segmentColor1} 
                onChange={e => updateSegmentColors(1, e.target.value)} 
                className="w-10 h-10 rounded border border-gray-300" 
              />
              <input 
                type="text" 
                value={segmentColor1} 
                onChange={e => updateSegmentColors(1, e.target.value)} 
                className="flex-1 px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#841b60] focus:border-transparent text-sm" 
              />
            </div>
          </div>
          
          <div className="space-y-2">
            <label className="text-sm font-medium text-gray-700">Couleur segments 2</label>
            <div className="flex items-center space-x-2">
              <input 
                type="color" 
                value={segmentColor2} 
                onChange={e => updateSegmentColors(2, e.target.value)} 
                className="w-10 h-10 rounded border border-gray-300" 
              />
              <input 
                type="text" 
        ...
        <p className="text-xs text-gray-500">
          Les segments alternent automatiquement entre ces deux couleurs
        </p>
      </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-2">
                  ...
                  <div 
                    ...
                    style={{ backgroundColor: index % 2 === 0 ? segmentColor1 : segmentColor2 }}
                  ></div>
                </div>
                <button 
