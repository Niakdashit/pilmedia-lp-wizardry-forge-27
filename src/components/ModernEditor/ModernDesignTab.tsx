...
const ModernDesignTab: React.FC<ModernDesignTabProps> = ({
  ...
  const renderTextStyleEditor = (key: string, style: any, title: string) => <div className="space-y-2 border rounded p-3">
      <h4 className="text-sm font-medium text-gray-800 mb-1">{title}</h4>
      <div className="grid grid-cols-2 gap-2 items-center">
        <label className="text-sm text-gray-700">Police</label>
        <select value={style.fontFamily || 'Inter'} onChange={e => handleTextStyleChange(key, 'fontFamily', e.target.value)} className="px-2 py-1 text-sm border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-[#841b60] focus:border-transparent">
          {fontOptions.map(opt => <option key={opt.value} value={opt.value}>
              {opt.label}
            </option>)}
        </select>
      </div>
      ...
      <details className="text-xs">
        ...
        <div className="mt-2 space-y-2">
          <div className="grid grid-cols-2 gap-2 items-center">
            ...
            <input type="text" value={style.fontSize || ''} onChange={e => handleTextStyleChange(key, 'fontSize', e.target.value)} className="px-2 py-1 text-sm border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-[#841b60] focus:border-transparent" placeholder="16px" />
          </div>
          <div className="grid grid-cols-2 gap-2 items-center">
            <label className="text-sm text-gray-700">Graisse</label>
            <select value={style.fontWeight || '400'} onChange={e => handleTextStyleChange(key, 'fontWeight', e.target.value)} className="px-2 py-1 text-sm border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-[#841b60] focus:border-transparent">
              {weightOptions.map(opt => <option key={opt.value} value={opt.value}>
                  {opt.label}
                </option>)}
            </select>
          </div>
          <div className="grid grid-cols-2 gap-2 items-center">
            ...
            <select value={style.textAlign || 'left'} onChange={e => handleTextStyleChange(key, 'textAlign', e.target.value)} className="px-2 py-1 text-sm border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-[#841b60] focus:border-transparent">
              {alignOptions.map(opt => <option key={opt.value} value={opt.value}>
                  {opt.label}
                </option>)}
            </select>
          </div>
          <div className="grid grid-cols-2 gap-2 items-center">
            ...
            <input type="text" value={style.lineHeight || ''} onChange={e => handleTextStyleChange(key, 'lineHeight', e.target.value)} className="px-2 py-1 text-sm border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-[#841b60] focus:border-transparent" placeholder="1.5" />
          </div>
        </div>
      </details>
    </div>;
  ...
  return <div className="space-y-4">
      ...
      <AccordionSection title="En-tête" icon={<Layout className="w-5 h-5 mr-2" />} bgColor="bg-gray-50">
        ...
        <div className="space-y-3">
          ...
          {headerBanner.enabled && <>
              <ImageUpload value={headerBanner.image || ''} onChange={value => handleHeaderBannerChange('image', value)} label="Image de bannière d'en-tête" compact={true} />
      ...
      <AccordionSection title="Centre & Arrière-plan" icon={<AlignCenter className="w-5 h-5 mr-2" />} bgColor="bg-blue-50">
        ...
        <div className="space-y-3">
          ...
          <ImageUpload value={campaign.design?.backgroundImage || ''} onChange={value => handleDesignChange('backgroundImage', value)} label="Image de fond" compact={true} />
        </div>
        ...
        {campaign.type === 'wheel' && <div className="space-y-3 border-t pt-3">
            ...
            <ImageUpload value={campaign.design?.centerLogo || ''} onChange={value => handleDesignChange('centerLogo', value)} label="Logo central" compact={true} />
          </div>}
        ...
        <div className="space-y-3 border-t pt-3">
          ...
          <div className="grid grid-cols-2 gap-2 items-center">
            ...
            <select value={selectedTextElement} onChange={e => setSelectedTextElement(e.target.value as any)} className="px-2 py-1 text-sm border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-[#841b60] focus:border-transparent">
              {elementOptions.map(opt => <option key={opt.value} value={opt.value}>
                  {opt.label}
                </option>)}
            </select>
          </div>

          {renderTextStyleEditor(selectedTextElement, (textStyles as any)[selectedTextElement] || {}, elementOptions.find(el => el.value === selectedTextElement)?.label || '')}
        </div>
      </AccordionSection>
      ...
      <AccordionSection title="Pied de page" icon={<MoreHorizontal className="w-5 h-5 mr-2" />} bgColor="bg-green-50">
        ...
        <div className="space-y-3 border-t pt-3">
          ...
          {footerBanner.enabled && <>
              <ImageUpload value={footerBanner.image || ''} onChange={value => handleFooterBannerChange('image', value)} label="Image de bannière de pied de page" compact={true} />
