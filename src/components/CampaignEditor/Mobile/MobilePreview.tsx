    ...
    switch (campaign.type) {
      case 'wheel':
        ...
        if (hasMobileRouletteConfig) {
          ...
          return <div className="flex items-center justify-center h-full w-full bg-gray-100 rounded-lg">
      ...
      case 'memory':
        return <div className="w-full h-full" style={{
          ...
          transformOrigin: 'center'
            ...
            <Memory config={campaign.gameConfig.memory} onConfigChange={() => {}} />
          </div>;
      case 'puzzle':
        return <div className="w-full h-full" style={{
          transform: 'scale(0.8)',
          transformOrigin: 'center'
            ...
            <Puzzle config={campaign.gameConfig.puzzle} onConfigChange={() => {}} />
          </div>;
      case 'dice':
        return <div className="w-full h-full" style={{
          transform: 'scale(0.8)',
      ...
      case 'jackpot':
        return <div className="w-full h-full" style={{
          ...
          transformOrigin: 'center'
            ...
            <Jackpot isPreview={true} instantWinConfig={campaign.gameConfig?.jackpot?.instantWin} onFinish={() => {}} />
          </div>;
      default:
        return <div className="flex items-center justify-center h-full w-full bg-white rounded-lg border-2 border-gray-200">
      ...
      <div style={getDeviceStyle()}>
        <div style={getScreenStyle()}>
          ...
          gap: `${mobileConfig.verticalSpacing || 16}px`
            ...
            {(showTitle || showDescription) && <div className="text-center space-y-2 flex-shrink-0 relative z-25">
                {showTitle && <h2 className={`${mobileConfig.titleSize || 'text-lg'} ${mobileConfig.titleWeight || 'font-bold'} ${mobileConfig.titleAlignment || 'text-center'}`} style={{
              color: mobileConfig.titleColor || '#000000',
              fontFamily: mobileConfig.fontFamily || 'Inter'
            }}>
                    {mobileConfig.title || campaign.name}
                  </h2>}
                {showDescription && <p className={`${mobileConfig.descriptionSize || 'text-sm'} ${mobileConfig.descriptionAlignment || 'text-center'}`} style={{
            ...
            <div style={getGameContainerStyle()} className="w-full flex flex-col items-center justify-center  gap-4">
            ...
            {mobileConfig.buttonText && <button className={`${mobileConfig.buttonShape || 'rounded-lg'} ${mobileConfig.buttonShadow || 'shadow-md'} transition-colors px-4 py-2 text-white font-medium flex-shrink-0 relative z-25`} style={{
