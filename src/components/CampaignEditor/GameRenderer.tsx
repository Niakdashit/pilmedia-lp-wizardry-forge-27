...
const GameRenderer: React.FC<GameRendererProps> = ({
  ...
  const validGamePosition = (['top', 'center', 'bottom', 'left', 'right'].includes(gamePosition) ? gamePosition : 'center') as 'top' | 'center' | 'bottom' | 'left' | 'right';
  switch (campaign.type) {
    case 'jackpot':
      return <div style={gameContainerStyle}>
          <Jackpot isPreview={true} instantWinConfig={{
    ...
    case 'wheel':
      return <div style={gameContainerStyle}>
          <WheelPreview campaign={campaign} config={{
          mode: 'instant_winner' as const,
          winProbability: campaign.gameConfig?.wheel?.winProbability || 0.1,
          maxWinners: campaign.gameConfig?.wheel?.maxWinners,
          winnersCount: 0
        }} onFinish={() => {}} gameSize={gameSize} gamePosition={validGamePosition} previewDevice={previewDevice} key={`${gameSize}-${gamePosition}-${previewDevice}-${JSON.stringify(campaign.gameConfig?.wheel)}`} />
        </div>;
    case 'scratch':
      return <div style={gameContainerStyle} className="mx-0">
          <ScratchPreview config={campaign.gameConfig?.scratch || {}} buttonLabel={buttonLabel} buttonColor={buttonColor} gameSize={gameSize} isPreview={true} />
        </div>;
    case 'memory':
      return <div style={gameContainerStyle}>
    ...
    case 'form':
      return <div style={gameContainerStyle}>
          <FormPreview campaign={campaign} gameSize={gameSize} />
        </div>;
    default:
