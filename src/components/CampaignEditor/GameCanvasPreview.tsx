    ...
    switch (campaign.type) {
      case 'jackpot':
        return <Jackpot isPreview={true} instantWinConfig={{
          ...
          winnersCount: 0
        }} buttonLabel={buttonLabel} buttonColor={buttonColor} hideDefaultTemplate={true} />;
      default:
        return null;
    }
  };
  return <div className={`bg-gray-100 rounded-lg p-6 border-2 border-dashed border-gray-300 ${className}`}>
      <div className="w-full max-w-3xl mx-auto relative" style={{
      minHeight: '500px'
    }}>
        {/* Container principal du jeu - dimensions fixes 680x400px, centré */}
        <div className="absolute bg-white rounded-lg shadow-lg overflow-hidden" style={{
        width: '680px',
        height: '400px',
        left: '50%',
        top: '50%',
        transform: 'translate(-50%, -50%)'
          ...
          {gameBackgroundImage && <img src={gameBackgroundImage} className="absolute inset-0 w-full h-full object-contain z-0" alt="Background" />}

          {/* Modèle de jackpot - s'affiche uniquement si uploadé (680x400) */}
          {jackpotTemplateImage && <img src={jackpotTemplateImage} className="absolute inset-0 w-full h-full object-contain z-1" alt="Jackpot Template" />}
          ...
          height: '100%'
            ...
            {renderGame() || <div className="text-center text-gray-500">
