...
const Sidebar: React.FC = () => {
  ...
  return (
    <div
      className={`fixed inset-y-0 left-0 z-10 flex flex-col bg-white/95 backdrop-blur-sm border-r border-gray-200/50 transition-all duration-300 ease-in-out ${
    ...
    >
      ...
      <div className="flex items-center justify-between h-16 px-4 border-b border-gray-200/50">
        {!sidebarCollapsed ? (
          <img src={logo} alt="Leadya Logo" className="h-14 w-auto" />
        ) : (
              ...
              <Link
                ...
                className={`flex items-center px-3 py-2 rounded-xl transition-all duration-200 group ${
                  isActive
                    ...
                    : 'text-gray-600 hover:bg-[#f8f0f5] hover:text-[#841b60]'
                }`}
              >
                <div className={`flex items-center justify-center w-8 h-8 rounded-lg ${
                  isActive ? 'bg-white/20' : 'bg-white group-hover:bg-white'
                }`}>
                  {item.icon}
                </div>
                {!sidebarCollapsed && (
