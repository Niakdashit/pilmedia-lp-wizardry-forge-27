    ...
    switch (campaign.type) {
      case 'wheel':
        return <WheelPreview campaign={campaign} config={{
          ...
          winnersCount: 0
        }} onFinish={handleGameComplete} onStart={handleGameStartInternal} gameSize={gameSize} gamePosition={gamePosition} previewDevice={previewMode} disabled={!formValidated} />;
      case 'scratch':
        return <ScratchPreview config={campaign.gameConfig?.scratch || {}} buttonLabel={buttonLabel} buttonColor={buttonColor} gameSize={gameSize} disabled={!formValidated} onFinish={handleGameComplete} onStart={handleGameStartInternal} isModal={isModal} />;
      case 'jackpot':
        return <Jackpot isPreview={true} instantWinConfig={campaign.gameConfig?.jackpot?.instantWin} buttonLabel={buttonLabel} buttonColor={buttonColor} backgroundImage={gameBackgroundImage} containerBackgroundColor={campaign.gameConfig?.jackpot?.containerBackgroundColor} backgroundColor={campaign.gameConfig?.jackpot?.backgroundColor} borderColor={campaign.gameConfig?.jackpot?.borderColor} borderWidth={campaign.gameConfig?.jackpot?.borderWidth} slotBorderColor={campaign.gameConfig?.jackpot?.slotBorderColor} slotBorderWidth={campaign.gameConfig?.jackpot?.slotBorderWidth} slotBackgroundColor={campaign.gameConfig?.jackpot?.slotBackgroundColor} onFinish={handleGameComplete} onStart={handleGameStartInternal} />;
      case 'quiz':
        return <div style={{
          width: '100%',
          maxWidth: '800px',
          ...
          justifyContent: 'center'
            ...
            <QuizPreview config={campaign.gameConfig?.quiz || {}} design={campaign.design} />
          </div>;
      case 'dice':
  ...
  return <div style={{
    ...
    overflow: 'hidden'
      ...
      height: '100%'
        ...
        <ContrastBackground enabled={contrastBg?.enabled && contrastBg?.applyToGame} config={contrastBg} className="flex items-center justify-center w-full h-full">
          <div style={{
          display: 'flex',
          alignItems: 'center',
          justifyContent: 'center',
