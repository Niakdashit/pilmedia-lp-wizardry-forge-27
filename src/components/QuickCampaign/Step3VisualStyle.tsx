        ...
        duration: 0.6
          ...
          <div className="bg-white/80 backdrop-blur-lg rounded-[32px] shadow-2xl p-12 border border-white/20">
            ...
            type: "spring"
              ...
              {creationSuccess ? <CheckCircle className="w-20 h-20 text-green-500 mx-auto" /> : <Sparkles className="w-20 h-20 text-[#841b60] mx-auto" />}
            </motion.div>
            ...
            {creationSuccess ? <p className="text-xl text-gray-600 mb-8">
                Redirection vers vos campagnes...
              </p> : <p className="text-xl text-gray-600 mb-8">
                Vous pouvez maintenant la tester ou la personnaliser davantage.
              </p>}

            {!creationSuccess && <div className="space-y-4">
                <motion.button whileHover={{
              scale: 1.02
            }} whileTap={{
              scale: 0.98
            }} onClick={handlePreview} className="w-full py-4 bg-gradient-to-r from-blue-500 to-cyan-500 text-white font-bold rounded-2xl hover:shadow-lg transition-all flex items-center justify-center space-x-3">
              ...
              scale: 0.98
            }} onClick={handleCreateCampaign} disabled={isCreating} className="w-full py-4 bg-gradient-to-r from-green-500 to-emerald-500 text-white font-bold rounded-2xl hover:shadow-lg transition-all disabled:opacity-50">
              ...
              scale: 0.98
            }} onClick={handleAdvancedSettings} disabled={isCreating} className="w-full py-4 bg-white/80 text-gray-700 font-bold rounded-2xl border-2 border-gray-200 hover:border-gray-300 hover:shadow-lg transition-all flex items-center justify-center space-x-3 disabled:opacity-50">
  ...
  return <div className="min-h-screen w-full flex items-center justify-center p-6">
      <div className="max-w-6xl w-full">
        ...
        delay: 0.2
          ...
          {/* Choix des thèmes */}
          <div>
            <h3 className="text-2xl font-bold text-gray-900 mb-8">Thèmes prédéfinis</h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {themes.map(theme => <motion.button key={theme.id} whileHover={{
              scale: 1.05
            }} whileTap={{
              scale: 0.95
            }} onClick={() => setSelectedTheme(theme.id)} className={`
                    p-8 rounded-3xl border-2 transition-all text-left
                    bg-white/60 backdrop-blur-sm shadow-lg
                    ${selectedTheme === theme.id ? `${theme.borderColor} border-4 ${theme.glowColor} shadow-xl` : 'border-gray-200 hover:border-gray-300 hover:shadow-xl'}
                  `}>
                  <div className={`w-full h-32 rounded-2xl mb-6 ${theme.preview} shadow-lg`} />
                  <h4 className="font-bold text-xl text-gray-900 mb-3">{theme.name}</h4>
          ...
          <div>
            ...
            <div className="border-2 border-dashed border-gray-300 rounded-3xl p-10 text-center bg-white/40 backdrop-blur-sm">
              ...
              {backgroundImage ? <div>
                  ...
                  <button onClick={() => setBackgroundImage(null)} className="text-gray-500 hover:text-red-500 mt-3 font-medium">
                    Supprimer
                  </button>
                </div> : <>
          ...
          <div className="flex justify-between items-center pt-8">
            ...
            scale: 0.95
          }} onClick={() => setCurrentStep(2)} className="flex items-center space-x-3 px-8 py-4 text-gray-600 hover:text-gray-900 transition-colors font-medium rounded-2xl hover:bg-gray-100">
            ...
            scale: 0.95
          }} onClick={handleFinish} disabled={!selectedTheme} className={`
                px-12 py-4 rounded-2xl font-bold text-lg transition-all
                ${selectedTheme ? 'bg-gradient-to-r from-[#841b60] to-pink-500 text-white hover:shadow-xl' : 'bg-gray-200 text-gray-400 cursor-not-allowed'}
              `}>
              Finaliser
            </motion.button>
          </div>
