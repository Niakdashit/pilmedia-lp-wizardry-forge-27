...
const JackpotSlot: React.FC<{
  ...
  slotBorderWidth: number;
  slotBackgroundColor: string;
}> = ({ isRolling, finalSymbol, slotBorderColor, slotBorderWidth, slotBackgroundColor }) => {
  ...
  return (
    ...
    >
      ...
      >
        {animationSymbols.map((symbol, index) => (
          <div
            key={index}
  ...
  return (
    <div className="flex flex-col items-center justify-center space-y-6 py-8">
      ...
      <div className="flex flex-col items-center space-y-4">
        {result && (
          <div className={`text-lg font-bold ${result === 'win' ? 'text-green-600' : 'text-red-500'}`}>
        ...
        <div className="flex space-x-3">
          <button
            onClick={roll}
            disabled={isRolling}
            className="px-6 py-3 text-white font-medium rounded-lg transition-all duration-200 disabled:opacity-50"
            style={{
