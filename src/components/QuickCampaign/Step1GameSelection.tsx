  ...
  return <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 flex items-center justify-center p-6 rounded-3xl">
      <div className="max-w-6xl w-full">
        ...
        duration: 0.6
          ...
          <h1 className="text-5xl font-bold text-[#841b60] mb-4">
            Quel type d'expérience voulez-vous créer ?
          </h1>
          <p className="text-xl text-[#841b60]/80 mb-2">
            Vous êtes à 2 clics de lancer une campagne interactive.
          </p>
          <p className="text-lg text-[#841b60]/60">
        ...
        delay: 0.2
          ...
          scale: 0.95
        }} onClick={() => handleGameSelect(game.id)} className={`relative p-8 rounded-2xl overflow-hidden bg-white/60 backdrop-blur-md shadow-lg hover:shadow-xl transition-all duration-300 cursor-pointer border-2 ${selectedGameType === game.id ? 'border-blue-500' : 'border-transparent'}`}>
              ...
              <div className="relative z-10">
                ...
                <button className={`w-full py-3 px-6 rounded-xl font-semibold transition-all duration-200 ${selectedGameType === game.id ? 'bg-blue-500 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
                  Choisir
                </button>
              </div>
            </motion.div>)}
